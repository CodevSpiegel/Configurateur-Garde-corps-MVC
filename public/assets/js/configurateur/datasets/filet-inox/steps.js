// ============================================================================
// 📋 CONFIGURATION DES ÉTAPES DU CONFIGURATEUR - FILET INOX
// ============================================================================
// Ce fichier définit toutes les étapes du parcours de configuration
// pour les filets inox (types 29, 30, 31 et 32)
// ============================================================================
...
export default [
  ...
  // ==========================================================================
  {
    ...
    // On part sur un filet inox standard, pose au sol, forme droite
    defaultPreview: "assets/images/configurateur/previews/filet-inox/filet-inox/sol/filet-inox-droit-sol.webp",

    // Liste des champs de formulaire de cette étape
    fields: [
      // --- Champ 1 : Type de filet inox ---
      {
        ...
        // Liste des 4 types de filets inox disponibles
        options: [
          {
            id: 29,                        // ID unique de l'option
            typeId: 29,                    // ID du type dans la base de données
            value: "filet-inox",           // Valeur technique (utilisée dans le code)
            label: "Filet inox",           // Texte affiché à l'utilisateur
            image: "assets/images/configurateur/types/filet-inox/filet-inox.webp" // Image de la carte
          },
          {
            id: 30,
            typeId: 30,
            value: "filet-inox-et-cables",
            label: "Filet Inox et Cables",  // Filet combiné avec des câbles
            image: "assets/images/configurateur/types/filet-inox/filet-inox-et-cables.webp"
          },
          {
            id: 31,
            typeId: 31,
            value: "filet-inox-et-barres",
            label: "Filet Inox et Barres",  // Filet combiné avec des barres
            image: "assets/images/configurateur/types/filet-inox/filet-inox-et-barres.webp"
          },
          {
            id: 32,
            typeId: 32,
            value: "filet-inox-muret",
            label: "Filet Inox Muret",      // Filet pour installation sur muret
            image: "assets/images/configurateur/types/filet-inox/filet-inox-muret.webp"
          }
        ]
      },
        ...
        options: [
          {
            ...
            label: "Inox 304 Brossé (Intérieur)",  // Finition pour usage intérieur
          ...
          },
          {
            ...
            label: "Inox 316L Brossé (Extérieur)",  // Finition pour usage extérieur
          ...
          },
          {
            ...
            finitionId: 3,
            value: "tube-inox-316",
            label: "Inox 316 Poli Miroir (Mer/Piscine)",  // Finition résistante à l'eau salée
    ...
    fields: [
      {
        ...
        options: [
          ...
          },
          {
            ...
            formeId: 2,
            value: "en-v",
            label: "En V",      // Forme en angle (2 côtés)
          ...
          },
          {
            ...
            formeId: 5,
            value: "en-s",
            label: "En S",      // Forme en S (3 côtés avec courbure)
          ...
          },
          {
            ...
            formeId: 6,
            value: "complexe",
            label: "Complexe",  // Forme personnalisée (devis sur mesure)
            image: "assets/images/configurateur/formes/complexe.webp"
          }
        ],

        // ⚙️ Fonction callback déclenchée quand l'utilisateur change de forme
        // Permet de valider que la pose sélectionnée est compatible avec la nouvelle forme
        onChange: ({ selection }) => {
          // Si aucune pose n'a été sélectionnée, pas de validation nécessaire
          if (!selection.pose) return;

          // On vérifie si la pose actuelle est toujours valide pour la nouvelle forme
          ...
          if (!isPoseValidForForme(selection.pose, selection.forme)) {
            ...
            // On pourrait aussi laisser undefined et le buildPreviewPath
            // utilisera "sol" par défaut, pour garder un aperçu fluide
          }
        }
      }
    ],
  ...
  // ==========================================================================
  {
    ...
    fields: [
      // --- Champ 1 : Type de pose ---
      {
        ...
        // 2 types de pose disponibles pour le filet inox
        options: [
          {
            id: 1,                  // ID unique
            poseId: 1,              // ID de la pose en BDD
            value: "sol",           // Valeur technique
            label: "Sol",           // Pose fixée au sol
            image: "assets/images/configurateur/poses/sol.webp"
          },
          {
            id: 2,
            poseId: 2,
            value: "lateral",       // Pose fixée latéralement (sur un mur/façade)
            label: "Latérale",
        ...
        options: [
          {
            ...
            label: "Goujon à frapper pour béton",  // Pour support en béton
          ...
          },
          {
            ...
            label: "Tirefonds pour bois",    // Pour support en bois
          ...
          },
          {
            ...
            label: "Tiges Filetées pour scellement Chimique",  // Pour supports difficiles
  ...
  // ==========================================================================
  {
    ...
    fields: [
      // --- Champ : Longueur A ---
      {
        ...
        // - Ne s'affiche PAS si forme = "complexe" (mesures sur devis)
      ...
      // --- Champ : Longueur C ---
      {
        ...
        // Condition d'affichage :
        // - S'affiche pour pose "sol" OU "lateral"
        // - S'affiche UNIQUEMENT pour formes "en-s" ou "en-u" (3 côtés)
        // - Ne s'affiche PAS si forme = "complexe"
      ...
      // --- Champ : Hauteur ---
      {
        ...
        // Condition d'affichage :
        // - S'affiche pour pose "sol" OU "lateral"
        // - La hauteur est toujours nécessaire pour ces deux types de pose
...
function buildPreviewPath(selection) {
  ...
  // On récupère le type sélectionné (ex: "filet-inox")
  const type = selection.type;

  // Variables commentées car non utilisées actuellement dans le path
  // mais peuvent être utiles pour des évolutions futures
  ...
  // assets/images/configurateur/previews/filet-inox/{TYPE}/{POSE}/{TYPE}-{FORME}-{POSE}.webp
  //
  // Exemple : 
  // assets/images/configurateur/previews/filet-inox/filet-inox/sol/filet-inox-en-l-sol.webp
  //
  // ⚠️ Note : L'arborescence inclut la pose dans le chemin, contrairement aux barres/câbles
  // qui n'ont que type et forme. Cela permet d'avoir des images différentes selon
  // que le filet est posé au sol ou latéralement.
 ...
 * Permet d'éviter des combinaisons invalides (ex: pose inclinée sur forme en U)
 *
...
function isPoseValidForForme(pose, forme) {
  ...
  // (une forme complexe ne peut pas être inclinée)
